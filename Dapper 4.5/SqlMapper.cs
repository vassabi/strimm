...
#pragma warning restore 0618
        ...
        internal static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates, bool removeUnused, IList<LiteralToken> literals)
        {
            ...
            // -------------------------- CHANGE START ---------------------------------------------------------------------------
            // MAX 09-15-2014: Added code to ignore computed attributes, like CreatedDate or LastUpdateDate that should be
            // set at the stored procedure level.
            var allProps = type.GetProperties().Where(p => p.GetIndexParameters().Length == 0);
            var nonCalcProps = new List<PropertyInfo>();
            allProps.ToList().ForEach(x =>
                {
                    if (!x.CustomAttributes.Any(y => y.AttributeType.Equals(typeof(ComputedAttribute))))
                    {
                        nonCalcProps.Add(x);
                    }
                });
            
            var propsArr = nonCalcProps.ToArray();  //type.GetProperties().Where(p => p.GetIndexParameters().Length == 0).ToArray();
            // -------------------------- END OF CHANGE ----------------------------------------------------------------------------
